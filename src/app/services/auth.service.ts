...
export class AuthService {
      ...
      .pipe(
        finalize(() => {
          storageRef.getDownloadURL().subscribe((downloadURL) => {
            ...
            this.store.dispatch(new UpdateImageAction(downloadURL));
  ...
  async profileUpdate(formData, uid) {
    const userRef: AngularFirestoreDocument<any> = this.afs.doc(`users/${uid}`);
    userRef.set(
      { displayName: formData.displayName, mobile: formData.mobile },
      {
        merge: true,
      }
    );
